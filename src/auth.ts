  ...
  return async (request, response, next) => {
    ...
    if (suppliedToken && oauthVerifier && config.oauthResourceUrl) {
      try {
        ...
        // Keep one stable challenge for invalid OAuth tokens.
      }
    }

    const challenge = config.oauthEnabled
      ? `Bearer realm="jeopsok", error="invalid_token", scope="mcp:tools", resource_metadata="${oauthResourceMetadataUrl(config)}"`
