...
export async function startHttpServer(config:AppConfig,services:McpServices):Promise<RunningHttpServer>{
  ...
  app.get("/health",(_request,response)=>response.json({status:"ok",service:"jeopsok",version:"0.2.0",transportMode:"mcp-2026-stateless",protocolRevision:"2026-07-28",legacyStatelessFallback:true,activeMcpSessions:0,activeMcpRequests,managedProcesses:services.processManager.list().length,accessProfile:config.accessProfile,filesystemRestricted:services.accessPolicy.filesystemRestricted,allowedRootCount:config.allowedRoots.length,commandExecutionEnabled:services.accessPolicy.commandExecutionEnabled,commandAllowlistMode:config.accessProfile==="full"?"unrestricted":config.accessProfile==="operator"?(config.allowedCommands.includes("*")?"unrestricted":"allowlist"):"disabled",unrestrictedHostAccess:services.accessPolicy.unrestrictedHostAccess,authentication:config.allowNoAuth&&!config.authToken?"upstream-or-none":"bearer"}));
