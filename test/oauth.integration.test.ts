...
describe("OAuth 2.1 MCP authorization", () => {
  ...
  beforeAll(async () => {
    temporaryDirectory = await mkdtemp(path.join(os.tmpdir(), "jeopsok-oauth-test-"));
  ...
  it("discovers, authorizes with PKCE, refreshes, revokes, and calls MCP tools", async () => {
    ...
    const client = new Client({ name: "oauth-integration-test", version: "1.0.0" }, { versionNegotiation: { mode: "auto" } });
