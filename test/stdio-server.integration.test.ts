...
describe("stdio entry", () => {
  it("serves the safe default workspace surface to a legacy client", async () => {
    const child = spawn(process.execPath, ["--import", "tsx", "src/stdio-server.ts"], { cwd: process.cwd(), env: { ...process.env, MCP_ALLOW_NO_AUTH: "true", MCP_DEFAULT_CWD: process.cwd() }, stdio: ["pipe","pipe","pipe"] });
    children.push(child);
    ...
    expect(init.result.serverInfo).toMatchObject({name:"jeopsok",version:"0.3.0"});
